...
##Changelog
## New baseline for throwing out data in IBI calculation. Self bias' based on the last heart rate
## Reliability estimate based on % to Zephyr
...
def findRelMax(buffer, threshold): ##Finds the first relative max
    for i in range (0,len(buffer)):
        if buffer[i]>threshold and i+50<len(buffer): ##Checks the relative max against the next 60 samples (63 samples per 250ms, there should be no peak higher than the selected value for atleast 60 samples
...
def calcRHR(peaks,prevHR):
    ...
    if prevHR != -1:
        prevIBI = (60/prevHR)*1000

        lBaseline = max(prevIBI-150, 290)
        hBaseline = min(prevIBI+150, 1500)

        #print("l: " + str(lBaseline) + "    h: " +str(hBaseline) + "   prevIBI: " + str(prevIBI)) #DEBUG



    if (len(peaks)>0):
        for i in range (1, len(peaks)):
            if peaks[i] - peaks[i-1]>lBaseline and peaks[i] - peaks[i-1]<hBaseline: #Filter out any IBIs that are outside the threshold
                #print("Peak #" + str(i) + ": " + str(peaks[i] - peaks[i-1])) #DEBUG
        ...
        if len(ibiBuffer)>0:
            ...
            if w!=0:
                ...
                if not math.isnan(60/(avg/w/1000)):
                    ...
                    if w!=0:
                        if not math.isnan(60 / (avg / w / 1000)):
                            return int(60 / (avg / w / 1000)), numReliable
                ...
                if w!=0:
                    if not math.isnan(60/(avg/w/1000)):
                        ...
                        return calcHR(peaks), numReliable
                else:
                    return calcHR(peaks), numReliable
        else:
...
def calcHR(peaks): ##OLD HR CALCULATION
    ...
    if numReliable>len(peaks)/3:
        return int(60/(avg/numReliable/1000))
    else:
        return -1
...
def pushMsg(r, ZephyrID):
    ...
    for i in ZephyrID: ##Push both HRs to the DcsLogger feed every second
        if i != "T":
            score = (ZephyrID[i][6]/ZephyrID[i][9]*100)-100
            s = "LPHR: " + str(ZephyrID[i][6]) + "   Zephyr: " + str(int(ZephyrID[i][9])) + "   % to Zephyr: " + str((ZephyrID[i][6]/ZephyrID[i][9]*100)-100)
            print(s)

            t = time.mktime(datetime.now().timetuple())*1000 + int(datetime.now().microsecond/1000)

            msg = OrderedDict([("timestamp", int(t)), ("device", "LPHR"), ("deviceId", str(i)), ("messageType", "refHR"),("data", [str(int(ZephyrID[i][6])), str((ZephyrID[i][6]/ZephyrID[i][9]*100)-100)])])
...
def ECG():
    ...
    while True:
        ...
        if temp != None:
            ...
            for i in ZephyrID:
                if i == str(temp["deviceId"]) or str(temp["deviceId"]) == "--" or str(temp["deviceId"] == "T"):
                    break
            else:
                numZephyr+=1
                ZephyrID[str(temp["deviceId"])] = [np.array([]),np.array([]),np.array([]),500, 0.95,pow(10, math.log10(0.95) * 0.25), -1,0, numZephyr, -1]
                #1st - ECG buffer
                #2nd times buffer
                #2rd peaks buffer
                ...
                #7th HR - init to -1
                #8th maxValTime
            if temp["device"] == 'BH3' and temp['messageType'] == 'GDP':
                ZephyrID[str(temp['deviceId'])][9] = temp['data'][0]
            if temp["device"] == 'BH3' and temp["messageType"] == 'EDP':
                ...
                if len(ZephyrID[currentZephyr][0])>6*63: ##When 10 sets of 63 samples are in the buffer, begin calculations
                    ...
                    if len(ZephyrID[currentZephyr][2])>=11: ##Adjust the number of peaks to detect from 11 to 2 for rough PtP calculation - Further refinement of PtP for HRV
                        ZephyrID[currentZephyr][6], numReliable = calcRHR(ZephyrID[currentZephyr][2], ZephyrID[currentZephyr][6])
                        ...
                        if len(ZephyrID[currentZephyr][2])>11:
                            ZephyrID[currentZephyr[2]] = clearBuffer(ZephyrID[currentZephyr][2],1) ##If the peaks buffer is greater than the desired number, clear extra
                        ...
                        if numReliable == 10 or numReliable == 9: # number of reliable peaks (IBI between 290 and 1500ms) is 9 or 10, further decrease the decay rate
                            if ZephyrID[currentZephyr][4]<0.95:
                                ZephyrID[currentZephyr][4] +=0.005
                            ZephyrID[currentZephyr][5] = pow(10, math.log10(ZephyrID[currentZephyr][4]) * 0.20)
                        elif numReliable <9  and numReliable>3: # Not enough reliable peaks, increase decay rate to catch more variation
                            if ZephyrID[currentZephyr][4] > 0.85:
                                ZephyrID[currentZephyr][4] -=0.005
                            ZephyrID[currentZephyr][5] = pow(10, math.log10(ZephyrID[currentZephyr][4]) * 0.20)

                    if ZephyrID[currentZephyr][7] != 0: ##If there is a clear peak found, clear buffer to that peak
                        ZephyrID[currentZephyr][0] = clearBuffer(ZephyrID[currentZephyr][0], ZephyrID[currentZephyr][7]+50)
                        ZephyrID[currentZephyr][1] = clearBuffer(ZephyrID[currentZephyr][1], ZephyrID[currentZephyr][7]+50)
...
Timer(10, pushMsg, (r, ZephyrID)).start()
